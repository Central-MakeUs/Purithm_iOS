//
//  ProfileTotalStampViewModel.swift
//  Profile
//
...
extension ProfileTotalStampViewModel {
    struct Input {
...
final class ProfileTotalStampViewModel {
    weak var coordinator: ProfileCoordinatorable?
    weak var usecase: ProfileUsecase?
    
    private let converter = ProfileTotalStampSectionConverter()
    ...
    private func handleAdapterActionEvent(input: Input, output: Output) {
        input.adapterActionEvent
            .sink { [weak self] actionItem in
                guard let self else { return }
                switch actionItem {
                case _ as ProfileMoveToAccessedFilterAction:
